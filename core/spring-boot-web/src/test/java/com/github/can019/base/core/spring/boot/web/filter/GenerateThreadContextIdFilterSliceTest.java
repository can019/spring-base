...
@WebMvcTest(controllers = {GenerateThreadContextIdSliceTestController.class},
        properties = {"spring.profiles.active=GENERATE_THREAD_CONTEXT_ID_TEST"})
@Import(GeneralThreadContextFilterTestConfig.class)
public class GenerateThreadContextIdFilterSliceTest {
    ...
    void GenerateThreadContextIdFilter가_먼저_결합된_경우_Thread_context에_id가_존재해야한다() {
        mockMvc = MockMvcBuilders.webAppContextSetup(context)
                .addFilters(new GenerateThreadContextIdFilter(), new ThreadContextSliceTestCheckFilter()).build();
    ...
    void GenerateThreadContextIdFilter가_나중에_결합된_경우_Thread_context에_id가_존재하면_안된다() {
        mockMvc = MockMvcBuilders.webAppContextSetup(context)
                .addFilters(new ThreadContextSliceTestCheckFilter(), new GenerateThreadContextIdFilter()).build();
        try {
            mockMvc.perform(get("/foo"));
        } catch (RuntimeException e) {
            assertThat(e.getMessage()).isEqualTo(ThreadContextSliceTestCheckFilter.EMPTY_THREAD_CONTEXT_MESSAGE);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
